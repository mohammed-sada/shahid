export async function getWachedVideos(token, { userId }) {
  const operationsDoc = `
  query getWachedVideos($userId:String!) {
    stats(where: {watched: {_eq: true}, userId: {_eq: $userId}}) {
      videoId
    }
  }
`;
  const result = await fetchGraphQL(operationsDoc, "getWachedVideos", { userId }, token);
  return result.data?.stats.length > 0 ? result.data?.stats : null;
}

export async function getMyListVideos(token, { userId }) {
  const operationsDoc = `
  query getMyListVideos($userId:String!) {
    stats(where: {favourited: {_eq: true}, userId: {_eq: $userId}}) {
      videoId
    }
  }
`;
  const result = await fetchGraphQL(operationsDoc, "getMyListVideos", { userId }, token);
  return result.data?.stats.length > 0 ? result.data?.stats : null;
}
